...
export default function PowerMeter() {
    ...
    const { data: power_meterValues, isLoading: isDataLoading } = useQuery({
        ...
        queryFn: async () => {
            ...
            const res = await fetch(`/api/admin/crud/power_meter?first=${lazyState.first}&rowcount=${lazyState.rows}&filter=${filters}`);
    ...
    const { data: count, isLoading: isCountLoading } = useQuery<number>({
        ...
        queryFn: async () => {
            const filters = encodeURIComponent(JSON.stringify(lazyState.filters));
            const res = await fetch(`/api/admin/crud/power_meter/count?filter=${filters}`);
    ...
    return (
        <div className="card">
            <h2>Power meter</h2>
            <Toast ref={toast} />
            <Dialog header="Power meter" visible={visible} onHide={() => setVisible(false)} style={{ width: '50vw' }}>
            ...
            <div className="card">
                <DataTable value={power_meterValues}
                    ref={dt}
                    header={header}
